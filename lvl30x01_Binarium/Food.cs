...
public class CodeDust : IConsumable
{
    public string? Name { get; private set; } = "Code Dust";

    public void Eat(Health health)
    {
        if (health.Status == Status.OnFire) health.Binarium += .01f;
    }
...
public class Health
{
    ...
    public float Binarium { get; set; } = 0.0f;
}
